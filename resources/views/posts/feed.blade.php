...
<div class="col-12">
    ...
    <div class="quick-actions">
        <button class="action-btn" onclick="location.href='{{ route('betting.index') }}'">
            <div class="action-icon">⚽</div>
            <div class="action-label">Match Betting</div>
        </button>
        <button class="action-btn" onclick="location.href='{{ route('matches.live') }}'">
            <div class="action-icon">🏆</div>
            <div class="action-label">All Matches</div>
        </button>
        <button class="action-btn" onclick="location.href='{{ route('profile.show', auth()->user()) }}'">
            <div class="action-icon">👤</div>
    ...
    <div class="row mt-4">
        ...
        <div class="col-lg-3 d-none d-lg-block">
            <div class="section-card sticky-left-nav">
                ...
                <a href="{{ route('clubs.index') }}" class="left-nav-item {{ Route::is('clubs.index') ? 'active' : '' }}">
                    <i class="fas fa-users text-info"></i> 
                    <span>Clubs</span> 
                </a>
                <a href="{{ route('matches.live') }}" class="left-nav-item"> {{-- Link to live matches --}}
                    <i class="fas fa-calendar-check text-danger"></i>
                    <span>Matches</span>
                </a>
                <a href="#" class="left-nav-item"> {{-- Placeholder for Saved --}}
        ...
        <div class="col-lg-6">
            @yield('feed_header') {{-- This is for the news page, will be empty on feed --}}
            @auth
                <div class="card section-card mb-4 bg-white ">
                    <div class="card-body">
                        <div class="d-flex align-items-center gap-3 mb-3">
                            <img src="{{ auth()->user()->profile_picture ? asset('storage/' . auth()->user()->profile_picture) : 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=random' }}" alt="{{ auth()->user()->name }}" class="avatar">
                            <button type="button" class="btn btn-outline-secondary flex-grow-1 text-start" data-bs-toggle="modal" data-bs-target="#createPostModal">
                                Drop Your Banta, {{ auth()->user()->name }}?
                            </button>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="modal-body px-4">
                            <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data" id="fbPostForm">
                                ...
                                <div class="fb-user-info">
                                    ...
                                    <div>
                                        <div class="user-name text-white">{{ auth()->user()->name }}</div>
                        ...
                        <div class="card post-card mb-3 text-white">
                            <div class="post-header d-flex justify-content-between align-items-start text-white">
                                <div class="d-flex align-items-center text-white">
                                    <img src="{{ $post->user->profile_picture ? asset('storage/' . $post->user->profile_picture) : 'https://ui-avatars.com/api/?name=' . urlencode($post->user->name) . '&background=random' }}" alt="{{ $post->user->name }}" class="avatar me-3 ">
                                    <div>
                                        <h6 class="mb-0 ">
                                            <a href="{{ route('profile.show', $post->user) }}" class="text-white text-decoration-none">{{ $post->user->name }}</a>
                                        </h6>
                                    <small class="text-muted">
                                        {{ '@' . $post->user->username }} · 
                                        @if($post->user->favoriteClub)
                                            <span class="text-white fw-semibold"><i class="fas fa-shield-alt small text-dark"></i> {{ $post->user->favoriteClub->name }}</span> ·
                                        @endif
                            ...
                            @if ($post->club_id)
                                <div class="px-3 pt-2 ">
                                    <span class="badge bg-info tex-white">{{ $post->club->name }}</span>
                                </div>
                            @endif
                            ...
                            <div class="post-footer">
                                @auth
                                    <button type="button" class="engagement-btn like-btn @if ($post->isLikedBy(auth()->user())) liked @endif" data-post-id="{{ $post->id }}" onclick="handleEngagement(this, 'like')">
                                        <i class="fas fa-thumbs-up "></i> <span>Like ({{ $post->likes_count }})</span>
                                    </button>
                                    <button type="button" class="engagement-btn dislike-btn @if ($post->isDislikedBy(auth()->user())) liked @endif" data-post-id="{{ $post->id }}" onclick="handleEngagement(this, 'dislike')">
                                        <i class="fas fa-thumbs-down"></i> <span>Dislike ({{ $post->dislikes_count }})</span>
                                    </button>
                                @else
                                    <button><i class="fas fa-thumbs-up"></i> Like ({{ $post->likes_count }})</button>
                                    <button><i class="fas fa-thumbs-down"></i> Dislike ({{ $post->dislikes_count }})</button>
                                @endauth
                                <a href="{{ route('posts.show', $post) }}" style="color: #6c757d; text-decoration: none;">
                                    <i class="fas fa-comment"></i> Comment ({{ $post->comments_count }})
                                </a>
                                <button><i class="fas fa-share"></i> Share ({{ $post->shares_count }})</button>
                            </div>
                        </div>
                @empty
                        <div class="card">
...
@section('scripts')
    ...
    <script>
        ...
        function quickCreateBet(matchTitle) {
            ...
            window.location.href = '{{ route('betting.index') }}';
        }
